using PagedList;
using DodoPetStore.Areas.Admin.Models.ViewModels.Shop;
    ...
    public class ShopController : Controller
    {
        ...
        public ActionResult AddProduct(ProductsVM model, HttpPostedFileBase file)
        {
            ...
            if (file != null && file.ContentLength > 0)
            {
                ...
                //Verify extension
                if (extention != "image/jpg" && extention != "image/jpeg" &&
                    extention != "image/pjpeg" && extention != "image/gif" &&
                    extention != "image/x-png" && extention != "image/png")
                {
                    using (Db db = new Db())
                    {
                        model.Categories = new SelectList(db.Categories.ToList(), "Id", "Name");
                        ModelState.AddModelError("", "Image failed to upload, please check image extension!");
                        return View(model);
                    }
                }
        ...
        public ActionResult Products(int? page, int? catId)
        {
            ...
            using (Db db = new Db())
            {
                ...
                listOfProductsVM = db.Products.ToArray()
                    ...
                    .Select(x => new ProductsVM(x)).ToList();
        ...
        public ActionResult EditProduct(ProductsVM model, HttpPostedFileBase file)
        {
            ...
            // Set TempData message
            TempData["SM"] = "Product has been edited!";
